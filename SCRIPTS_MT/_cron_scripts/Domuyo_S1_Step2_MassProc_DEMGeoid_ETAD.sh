#!/bin/bash
# Script to run in cronjob for processing Domuyo images with EATD data:
...
TABLEASC=$PATH_1650/SAR_SM/MSBAS/ARGENTINE/set11/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After_WITHHEADER.txt	# exist anyway even if first ETAD is after DATECHG because copied at cron step 1
TABLEDESC=$PATH_1650/SAR_SM/MSBAS/ARGENTINE/set12/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After_WITHHEADER.txt	# exist anyway even if first ETAD is after DATECHG because copied at cron step 1
#TABLEASC=$PATH_1650/SAR_SM/MSBAS/ARGENTINE/set11/table_0_${BP2}_0_${BT2}.txt	# exist anyway even if first ETAD is after DATECHG because copied at cron step 1
#TABLEDESC=$PATH_1650/SAR_SM/MSBAS/ARGENTINE/set12/table_0_${BP2}_0_${BT2}.txt	# exist anyway even if first ETAD is after DATECHG because copied at cron step 1

#Launch param files
PARAMPROCESSASC=$PATH_1650/Param_files/S1/ARG_DOMU_LAGUNA_A_18/LaunchMTparam_S1_Arg_Domu_Laguna_A_18_Zoom1_ML4_MassProc_MaskCohWater_DEMGeoid_ETAD.txt
PARAMPROCESSDESC=$PATH_1650/Param_files/S1/ARG_DOMU_LAGUNA_D_83/LaunchMTparam_S1_Arg_Domu_Laguna_D_83_Zoom1_ML4_MassProc_Snaphu_WaterCohMask_DEMGeoid_ETAD.txt
...
CRONSTEP1="Domuyo_S1_Step1_Read_SMCoreg_Pairs_DEMGeoid_ETAD.sh"
