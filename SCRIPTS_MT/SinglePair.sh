...
#              - SLV date or S1 name (accept both date or name for S1)
...
#                  if need coregistration on a super master, a 5th parameter 
...
# -----------------------------------------------------------------------------------------
PRG=`basename "$0"`
VER="Distro V5.0.0 MasTer script utilities"
AUT="Nicolas d'Oreye, (c)2016-2019, Last modified on Aug 28, 2023"
echo " "
echo "${PRG} ${VER}, ${AUT}"
...
echo "Last created MasTer Engine source dir suggest SinglePair processing with ME version: ${LASTVERSIONCIS}" > ${RUNDIR}/Processing_Pair_w_MasTerEngine_V.txt
...
if [ $# -eq 5 ] && [ "${S1MODE}" != "WIDESWATH" ] # && [ ${SATDIR} != "S1" ] 
	then 
		...
		then 
			...
			SpeakOut "Do you want to benefit from the coregistration of the master and slave on the super master to spare processing time ?" 
			while true; do
				...
				case $yn in
					[Yy]* ) 
						if [ ${SUPERMASTER} != "" ] 
							then 
								EchoTee "OK let's spare time and use the SuperMaster from your parameters file, that is ${SUPERMASTER}."
...
if [ "${SATDIR}" == "S1" ] && [ "${S1MODE}" == "WIDESWATH" ] && [ "${ZOOM}" != "1" ] 
	then
		EchoTee " Skip compute amplitude image of master Zoomed x ML for S1 WS Zoomed - may want to update script later for that"
