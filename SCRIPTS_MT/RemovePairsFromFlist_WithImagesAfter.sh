...
# Print the header
head -n 2 ${PAIRFILE} > ${PAIRFILE}_Before${MAXDATE}_WithBaselines_${RNDM}.txt 

# Filter lines based on the given date
awk -v given_date="${MAXDATE}" 'NR > 2 && ($1 < given_date && $2 < given_date) {print}' ${PAIRFILE} >> ${PAIRFILE}_Before${MAXDATE}_WithBaselines_${RNDM}.txt
