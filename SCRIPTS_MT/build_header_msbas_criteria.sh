...
# New in Distro V 4.8:	- check if more than one table_0_x_0_x_AdditionalPairs.txt files and if baselines criteria are satisfied
# New in Distro V 4.9:	- remove log files older than 30 days
# New in Distro V 4.9.1: - search for img date from file name with 8 digits instead of 6. Should be OK as long as crop is not defined with 8 decimals
...
# New in Distro V 5.1:  - set quotes in if test when checking for NaN to prevent prblms
# New in Distro V 5.2:  - remove empty lines from header, incid and acq time files to avoid error when computing average 
# New in Distro V 6.0:  - Run all modes in parallel
#						- remove undefined search for SATDIR
#						- set i between brackets to allow more than 9 sets
