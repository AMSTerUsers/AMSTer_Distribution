...
# New in Distro V 2.2.0 20260115:	- in check running process, do not take into account Crons_1_2_3.sh 
...
# New in Distro V 2.3.0 2026730 :	- force msbasv4								
#
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		LABEL=Funu3D 	# Label for file naming (used for naming zz_ dirs with results and figs etc)
	...
	CRONJOB2D=Funu_S1_Step3_MSBAS_shortest_2D.sh
...
###########################################################################################
	...
	if ! find "${PATH_DIR_FOR_FLAG}" -maxdepth 1 -name "Running_crons_${TARGET}*" -print -quit | grep -q .
		then
    		...
    		echo "start cron 3 3D at $(date '+%Y-%m-%d %H:%M:%S')" >> "${FLAGFILE}"
...
#	${PATH_SCRIPTS}/SCRIPTS_MT/build_header_msbas_criteria.sh ${DEFOMODE} 2 ${BP} ${BT} ${S1ASC} ${S1DESC}
	...
	${PATHGNU}/gsed -i 's/WINDOW_SIZE = 0, 330, 0, 369/WINDOW_SIZE = 0, 205, 100, 230/' ${MSBASDIR}/header.txt
...
##########
	...
	#----------------------------------------
 		...
 		then 
			...
			Plot_vectorDispl_DEM.py ${WHEREDEM} ${NSDISPL} ${EWDISPL}  --x_min 50 --x_max 115 --y_min 100 --y_max 240
