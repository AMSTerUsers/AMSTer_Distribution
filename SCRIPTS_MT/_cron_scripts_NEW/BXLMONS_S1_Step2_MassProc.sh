...
#mode SM
TABLEASC=$PATH_1650/SAR_SM/MSBAS/BXLMONS/set1/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After.txt		# do not take _WITHHEADER.txt because it does not contains the AddirionbalPairs
TABLEDESC=$PATH_1650/SAR_SM/MSBAS/BXLMONS/set2/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After.txt	# do not take _WITHHEADER.txt because it does not contains the AddirionbalPairs


PARAMPROCESSASC=$PATH_1650/Param_files/S1/BXLMONS_A_161/LaunchMTparam_S1_IW_BxlMons_A_Zoom1_ML2_MassProc.txt
PARAMPROCESSDESC=$PATH_1650/Param_files/S1/BXLMONS_D_110/LaunchMTparam_S1_IW_BxlMons_D_Zoom1_ML2_MassProc.txt

PARAMASCNAME=`basename ${PARAMPROCESSASC}`
PARAMDESCNAME=`basename ${PARAMPROCESSDESC}`

STEP1="BXLMONS_S1_Step1_Read_Coreg_Pairs.sh"

SMASC=20190405
SMDESC=20210310

MASSPROCDIRASC=$PATH_3611/SAR_MASSPROCESS/S1/BXLMONS_A_161/SMNoCrop_SM_${SMASC}_Zoom1_ML2
MASSPROCDIRDESC=$PATH_3611/SAR_MASSPROCESS/S1/GBXLMONS_D_110/SMNoCrop_SM_${SMDESC}_Zoom1_ML2
...
if [ ${CHECKREAD} -eq 0 ] 
	then 
		...
		echo "Step2 aborted on ${TODAY} because BXLMONS_S1_Step1_Read_Coreg_Pairs.sh is still running: wait for tomorrow"  >>  ${MASSPROCDIRASC}/_aborted_because_Read_inProgress.txt
		echo "Step2 aborted on ${TODAY} because BXLMONS_S1_Step1_Read_Coreg_Pairs.sh is still running: wait for tomorrow"  >> ${MASSPROCDIRDESC}/_aborted_because_Read_inProgress.txt
