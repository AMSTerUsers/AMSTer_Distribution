...
#			 - suppose everywhere that modes are ${MSBASMODE}
#
# Dependencies:	- 
# New in Distro V 1.0 20250122: - Adapt from VVP CSK to Guadeloupe TSX data
# New in Distro V 1.0.1 20250424 :	- check if correct termination in MSBAS_LOG.txt at right place
...
# -----------------------------------------------------------------------------------------
PRG=`basename "$0"`
VER="Distro V1.0.1 AMSTer script utilities"
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		BP=300			# max perpendicular baseline 

		#BTASC=150
		#BTDESC=200
		BT=150			# max temporal baseline

		LABEL=Guadeloupe 	# Label for file naming (used for naming zz_ dirs with results and figs etc)
		...
		MSBASMODE='DefoInterpolx2Detrend'
		MSBASMODE='COR_Defo_Dem'
		STARTDATE='20231201'

	# some files and PATH for each mode
	###################################
		# Path to Pair Dirs and Geocoded files to use (need one for each mode)
#		TSXASC=/Users/delphine/Documents/Guadeloupe/Data/SMNoCrop_SM_20240204_Zoom1_ML5
#		TSXDESC=/Users/delphine/Documents/Guadeloupe/Data/SMNoCrop_SM_20240724_Zoom1_ML5

		TSXASC=${PATH_3610}/SAR_MASSPROCESS/TSX/GUADELOUPE_A104/SMNoCrop_SM_20240204_Zoom1_ML5
		TSXDESC=${PATH_3610}/SAR_MASSPROCESS/TSX/GUADELOUPE_D20/SMNoCrop_SM_20240724_Zoom1_ML5

		# Path to dir where list of compatible pairs files are computed (need one for each mode)
		SET1=${PATH_1650}/SAR_SM/MSBAS/GUADELOUPE/set3
		SET2=${PATH_1650}/SAR_SM/MSBAS/GUADELOUPE/set4

		# Name of LaunchParameters.txt files for each mode (need one for each mode)
		LAUNCHPARAMASC=LaunchMTparameters_TSX_SPTL_A104_Guadeloupe_Full_Zoom1_ML5_MassProc.txt
		LAUNCHPARAMDESC=LaunchMTparameters_TSX_SPTL_D20_Guadeloupe_Full_Zoom1_ML5_MassProc.txt
...
#		fi
	...
	########################		
		...
		if [ ${IFCOH} == "YES" ]
			then 
				...
				KMLCOH=${PATH_1650}/kml/Guadeloupe/Souf_summit.kml
				...
				# ${MSBASDIR}/${MSBASMODE}i/_EXCLUDE_PAIRS_ALTHOUGH_CRITERIA_OK.txt
	...
	###########################################
		#MSBASDIR=${PATH_3602}/MSBAS/_${LABEL}_TSX_Auto_${BP}m_${BT}days_cordefo_restrict20231201_applycoh0325
		MSBASDIR=${PATH_3602}/MSBAS/_${LABEL}_TSX_Auto_${BP}m_${BT}days_${MSBASMODE}_restrict${STARTDATE}_applycoh${COHRESTRICT}
...
#		TIMESERIESPTSDESCR=${PATH_1650}/Data_Points/Points_TS_${LABEL}.txt
		TIMESERIESPTSDESCR=${PATH_1650}/Data_Points/Points_TS_${LABEL}withGNSS_TSX.txt
		...
		DOUBLEDIFFPAIRSEWUD=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_${LABEL}_TSX.txt
		DOUBLEDIFFPAIRSASC=${PATH_1650}/Data_Points/List_DoubleDiff_Asc_${LABEL}_TSX.txt
		DOUBLEDIFFPAIRSDESC=${PATH_1650}/Data_Points/List_DoubleDiff_Desc_${LABEL}_TSX.txt
		
		
	# Name of previous cron jobs for the automatic processing of that target (used to check that no other process is runing)
	#########################################################################
	CRONJOB2=GUADELOUPE_TSX_Step2_MassProc.sh
...
##################################################################
	...
	#LASTASC=`ls -lt ${TSXASC}/Geocoded/${MSBASMODE} | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'` # may be messing up if txt files are created for any other purpose in the dir... 
	#LASTDESC=`ls -lt ${TSXDESC}/Geocoded/${MSBASMODE} | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'`
	LASTASC=`find ${TSXASC}/Geocoded/${MSBASMODE}/ -maxdepth 1 -type f -name "*deg" -printf "%T+ %p\n" | sort -r | head -1 | ${PATHGNU}/gawk '{print $2}'`  
	LASTDESC=`find ${TSXDESC}/Geocoded/${MSBASMODE}/ -maxdepth 1 -type f -name "*deg" -printf "%T+ %p\n" | sort -r | head -1 | ${PATHGNU}/gawk '{print $2}'`
...
################################################################################
	if [ -f  "${MSBASDIR}/_Last_MassProcessed_Pairs_Time.txt" ] && [ -s  "${MSBASDIR}/_Last_MassProcessed_Pairs_Time.txt" ]
		then   
			...
			FORMERLASTDESCTIME=`tail -1 ${MSBASDIR}/_Last_MassProcessed_Pairs_Time.txt`

			# if no more recent file is available since the last cron processing
			if [ ${FORMERLASTASCTIME} -eq ${LASTASCTIME} ] && [ ${FORMERLASTDESCTIME} -eq ${LASTDESCTIME} ]  
				then
...
# (clean if required MODEi.txt and Checked_For_CohThreshold_To_Be_Ignored_At_Next_Rebuild_msbas_Header.txt if any)
	if [ "${FIRSTRUN}" == "NO" ] ; then 
		echo "Remove Broken Links and Clean txt file in existing ${MSBASDIR}/${MSBASMODE}"
		Remove_BrokenLinks_and_Clean_txt_file.sh ${MSBASDIR}/${MSBASMODE}1 &
 		Remove_BrokenLinks_and_Clean_txt_file.sh ${MSBASDIR}/${MSBASMODE}2 &
		wait
		...
		if [ ${IFCOH} == "YES" ] ; then 
			Remove_BrokenLinks_and_Clean_txt_file.sh ${MSBASDIR}/${MSBASMODE}1_Full &
			Remove_BrokenLinks_and_Clean_txt_file.sh ${MSBASDIR}/${MSBASMODE}2_Full &
			wait
...
# Remove possible lines with less that 4 columns
	if [ "${FIRSTRUN}" == "NO" ] ; then 
		mv ${MSBASMODE}1.txt ${MSBASMODE}1_all4col.txt
 		mv ${MSBASMODE}2.txt ${MSBASMODE}2_all4col.txt

		${PATHGNU}/gawk 'NF>=4' ${MSBASMODE}1_all4col.txt > ${MSBASMODE}1.txt 
 		${PATHGNU}/gawk 'NF>=4' ${MSBASMODE}2_all4col.txt > ${MSBASMODE}2.txt 
	
		rm -f ${MSBASMODE}1_all4col.txt ${MSBASMODE}2_all4col.txt 2>/dev/null
		echo "All lines in former existing MODEi.txt have 4 columns"
		echo ""

		#Need also for the _Full ones (that is without coh threshold)
		if [ ${IFCOH} == "YES" ] ; then 
			mv ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full_all4col.txt
			mv ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full_all4col.txt
			${PATHGNU}/gawk 'NF>=4' ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full_all4col.txt > ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt 
			${PATHGNU}/gawk 'NF>=4' ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full_all4col.txt > ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt 
			rm -f ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full_all4col.txt ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full_all4col.txt
...
# Remove lines in MSBAS/MODEi.txt file associated to possible broken links or duplicated lines with same name though wrong BP (e.g. after S1 orb update) 
		...
		_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}1 ${PATH_3601}/SAR_MASSPROCESS &
 		_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}2 ${PATH_3601}/SAR_MASSPROCESS &
		wait
		...
		#Need also for the _Full ones (that is without coh threshold)
		if [ ${IFCOH} == "YES" ] ; then 
			_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}1_Full ${PATH_3601}/SAR_MASSPROCESS &
			_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}2_Full ${PATH_3601}/SAR_MASSPROCESS &
			wait
...
# Prepare MSBAS
###############
	${PATH_SCRIPTS}/SCRIPTS_MT/build_header_msbas_criteria.sh ${MSBASMODE} 2 ${BP} ${BT} ${TSXASC} ${TSXDESC}

	# Restrict DATES 
	################
		echo " Because of data low density in Guadeloupe data before ${STARTDATE} in Descending mode, let's remove the data before that day in Ascending mode"
		RemovePairsFromModeList_WithImagesBefore.sh ${MSBASMODE}1.txt ${STARTDATE}		
		echo " Because of data gap in Guadeloupe data before ${STARTDATE} in Descending mode, let's remove the data before that day in Descending mode"
		RemovePairsFromModeList_WithImagesBefore.sh ${MSBASMODE}2.txt ${STARTDATE}		
		
		# rename files accordingly 
		mv -f ${MSBASMODE}1.txt ${MSBASMODE}1_FullDates.txt
		mv -f ${MSBASMODE}2.txt ${MSBASMODE}2_FullDates.txt
		FILECLEANED1=`ls -1tr ${MSBASMODE}1.txt_After${STARTDATE}*.txt | tail -1` # take the most recent if former run was not deleted
		FILECLEANED2=`ls -1tr ${MSBASMODE}2.txt_After${STARTDATE}*.txt | tail -1` # take the most recent if former run was not deleted
		cp ${FILECLEANED1} ${MSBASMODE}1.txt
		cp ${FILECLEANED2} ${MSBASMODE}2.txt
		 # Remember that EW-UD decomposition will be restricted to overlapping epoch...
	...
	# Check again that files are OK
		...
		${PATHGNU}/gawk 'NF<4' ${MSBASMODE}1_all4col.txt > ${MSBASMODE}1_MissingCol.txt 
 		${PATHGNU}/gawk 'NF<4' ${MSBASMODE}2_all4col.txt > ${MSBASMODE}2_MissingCol.txt 

		rm -f ${MSBASMODE}1_all4col.txt ${MSBASMODE}2_all4col.txt 
	...
	#----------------------------------------
		case ${FIRSTRUN} in 
			"YES") 
				...
				cp -R ${MSBASDIR}/${MSBASMODE}1 ${MSBASDIR}/${MSBASMODE}1_Full
				cp -f ${MSBASDIR}/${MSBASMODE}1.txt ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt
				cp -f ${MSBASDIR}/${MSBASMODE}1.txt ${MSBASDIR}/${MSBASMODE}1_Full.txt
				
				cp -R ${MSBASDIR}/${MSBASMODE}2 ${MSBASDIR}/${MSBASMODE}2_Full
				cp -f ${MSBASDIR}/${MSBASMODE}2.txt ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt
				cp -f ${MSBASDIR}/${MSBASMODE}2.txt ${MSBASDIR}/${MSBASMODE}2_Full.txt
				;;
			"NO")
				# one must merge the newly created MODEi dir and MODEi.txt with former _Full ones
				sort ${MSBASDIR}/${MSBASMODE}1.txt | uniq > ${MSBASDIR}/${MSBASMODE}1_tmp.txt
				sort ${MSBASDIR}/${MSBASMODE}2.txt | uniq > ${MSBASDIR}/${MSBASMODE}2_tmp.txt
				
				sort ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt | uniq > ${MSBASDIR}/${MSBASMODE}1_Full_tmp.txt
				sort ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt | uniq > ${MSBASDIR}/${MSBASMODE}2_Full_tmp.txt
				
				cat ${MSBASDIR}/${MSBASMODE}1_tmp.txt ${MSBASDIR}/${MSBASMODE}1_Full_tmp.txt | sort | uniq >  ${MSBASDIR}/${MSBASMODE}1_Full.txt
				cat ${MSBASDIR}/${MSBASMODE}2_tmp.txt ${MSBASDIR}/${MSBASMODE}2_Full_tmp.txt | sort | uniq >  ${MSBASDIR}/${MSBASMODE}2_Full.txt
				
				cp -R -n ${MSBASDIR}/${MSBASMODE}1 ${MSBASDIR}/${MSBASMODE}1_Full
				cp -R -n ${MSBASDIR}/${MSBASMODE}2 ${MSBASDIR}/${MSBASMODE}2_Full
				cp -f ${MSBASDIR}/${MSBASMODE}1_Full.txt ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt
				cp -f ${MSBASDIR}/${MSBASMODE}2_Full.txt ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt
				
				rm -f ${MSBASDIR}/${MSBASMODE}1_tmp.txt ${MSBASDIR}/${MSBASMODE}1_Full_tmp.txt 
				rm -f ${MSBASDIR}/${MSBASMODE}2_tmp.txt ${MSBASDIR}/${MSBASMODE}2_Full_tmp.txt
				;;	
		esac
		# trick the header file						
		${PATHGNU}/gsed -i 's/${MSBASMODE}1.txt/${MSBASMODE}1_Full.txt/' ${MSBASDIR}/header.txt
		${PATHGNU}/gsed -i 's/${MSBASMODE}2.txt/${MSBASMODE}2_Full.txt/' ${MSBASDIR}/header.txt

		${PATH_SCRIPTS}/SCRIPTS_MT/MSBAS.sh _Auto_${ORDER}_${LAMBDA}_${LABEL}_NoCohThresh ${TIMESERIESPTS}

		# Make baseline plot 
		PlotBaselineGeocMSBASmodeTXT.sh ${SET1} ${MSBASDIR}/${MSBASMODE}1_Full/${MSBASMODE}1_Full.txt
		PlotBaselineGeocMSBASmodeTXT.sh ${SET2} ${MSBASDIR}/${MSBASMODE}2_Full/${MSBASMODE}2_Full.txt
 	...
 	# EW-UD with coh threshold restriction 
  	#--------------------------------------
         ...
         # run restrict_msbas_to_Coh.sh         
          restrict_msbas_to_Coh.sh ${MSBASMODE}1 ${COHRESTRICT} ${KMLCOH} ${TSXASC}/Geocoded/Coh
          restrict_msbas_to_Coh.sh ${MSBASMODE}2 ${COHRESTRICT} ${KMLCOH} ${TSXDESC}/Geocoded/Coh
 		
 		# Force pair exclusion 
 			if [ ${EXCLUDE1} == "YES" ] ; then 
 				${PATH_SCRIPTS}/SCRIPTS_MT/zz_Utilities_MT/Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/${MSBASMODE}1
 			fi 
 			if [ ${EXCLUDE2} == "YES" ] ; then 
 				${PATH_SCRIPTS}/SCRIPTS_MT/zz_Utilities_MT/Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/${MSBASMODE}2
 			fi 
 			if [ ${EXCLUDE3} == "YES" ] ; then 
 				${PATH_SCRIPTS}/SCRIPTS_MT/zz_Utilities_MT/Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/${MSBASMODE}3
 			fi 
 			if [ ${EXCLUDE4} == "YES" ] ; then 
 				${PATH_SCRIPTS}/SCRIPTS_MT/zz_Utilities_MT/Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/${MSBASMODE}4
 			fi 
		cd ${MSBASDIR}
 		...
 		# test if MSBAS_log.txt contains "completed 100%" ; if not log error 
   		...
   		#		echo "MSBAS ok" 
   		#	else 
   		#		# try again after cleaning ${MSBASMODE}i.txt
   		#		_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}1 ${PATH_3601}/SAR_MASSPROCESS &
   		#		_Check_bad_DefoInterpolx2Detrend.sh ${MSBASMODE}2 ${PATH_3601}/SAR_MASSPROCESS &
   		#		wait 
   		#		
   		#		${PATH_SCRIPTS}/SCRIPTS_MT/MSBAS.sh _Auto_${ORDER}_${LAMBDA}_${LABEL} ${TIMESERIESPTS}
   		#		if ${PATHGNU}/grep -q "writing results to a disk" ${MSBASDIR}/zz_EW_Auto_${ORDER}_${LAMBDA}_${LABEL}/MSBAS_LOG.txt ; then echo "Solved after cleaning ${MSBASMODE}'s txt"; else  echo "!! MSBAS crashed on ${TODAY}"  >>  ${MSBASDIR}/_last_MSBAS_process.txt ; fi
   		#fi
 
 		# Make baseline plot 
		PlotBaselineGeocMSBASmodeTXT.sh ${SET1} ${MSBASDIR}/${MSBASMODE}1.txt
		PlotBaselineGeocMSBASmodeTXT.sh ${SET2} ${MSBASDIR}/${MSBASMODE}2.txt
 	...
 	#--------------
		...
		rm -f ${MSBASDIR}/header_none.txt 2> /dev/null
 		...
 		#  DESC
			...
			# rename files accordingly
			 mv -f ${MSBASMODE}2_Full.txt ${MSBASMODE}2.txt
