...
#
#
# New in Distro V 1.0.0 20250411 :	- based on Galeras and Nepal processing
#
...
#mode SM
#TABLEASC=$PATH_1660/SAR_SM/MSBAS/MUSTANG/set1/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After.txt	# do not take _WITHHEADER.txt because it does not contains the AddirionbalPairs
#TABLEDESC=$PATH_1660/SAR_SM/MSBAS/MUSTANG/set2/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After.txt	# do not take _WITHHEADER.txt because it does not contains the AddirionbalPairs
TABLEASC=$PATH_1660/SAR_SM/MSBAS/MUSTANG/set1/table_0_0_MaxShortest_3.txt
TABLEDESC=$PATH_1660/SAR_SM/MSBAS/MUSTANG/set2/table_0_0_MaxShortest_3.txt

PARAMPROCESSASC=$PATH_1650/Param_files/S1/MUSTANG_A_158/LaunchMTparam_S1_Mustang_A_158_Zoom1_ML2_MassProc_0keep.txt
PARAMPROCESSDESC=$PATH_1650/Param_files/S1/MUSTANG_D_19/LaunchMTparam_S1_Mustang_D_19_Zoom1_ML2_MassProc_0keep.txt


PARAMASCNAME=`basename ${PARAMPROCESSASC}`
PARAMDESCNAME=`basename ${PARAMPROCESSDESC}`

STEP1="MUSTANG_S1_Step1_Read_Coreg_Pairs.sh"

SMASC=20241116		# Asc 158
SMDESC=20190830		# Desc 19

MASSPROCDIRASC=$PATH_3611/SAR_MASSPROCESS/S1/MUSTANG_A_158/SMNoCrop_SM_${SMASC}_Zoom1_ML2
MASSPROCDIRDESC=$PATH_3611/SAR_MASSPROCESS/S1/MUSTANG_D_19/SMNoCrop_SM_${SMDESC}_Zoom1_ML2
...
if [ ${CHECKREAD} -eq 0 ] 
	then 
		...
		echo "Step2 aborted on ${TODAY} because MUSTANG_S1_Step1_Read_Coreg_Pairs.sh is still running: wait for tomorrow"  >>  ${MASSPROCDIRASC}/_aborted_because_Read_inProgress.txt
		echo "Step2 aborted on ${TODAY} because MUSTANG_S1_Step1_Read_Coreg_Pairs.sh is still running: wait for tomorrow"  >> ${MASSPROCDIRDESC}/_aborted_because_Read_inProgress.txt
