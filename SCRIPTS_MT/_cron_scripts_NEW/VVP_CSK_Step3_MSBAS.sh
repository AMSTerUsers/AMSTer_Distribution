...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		BP=151			# max perpendicular baseline 

		#BTASC=150
		#BTDESC=200
		BT=200			# max temporal baseline
	...
	###################################
		...
		CSKASC=${PATH_3602}/SAR_MASSPROCESS_2/CSK/Virunga_Asc/SMNoCrop_SM_20160627_Zoom1_ML23
		CSKDESC=${PATH_3602}/SAR_MASSPROCESS_2/CSK/Virunga_Desc/SMNoCrop_SM_20160105_Zoom1_ML23

		# Path to dir where list of compatible pairs files are computed (need one for each mode)
		SET1=${PATH_1650}/SAR_SM/MSBAS/VVP/set1
		SET2=${PATH_1650}/SAR_SM/MSBAS/VVP/set2

		# Name of LaunchParameters.txt files for each mode (need one for each mode)
		LAUNCHPARAMASC=LaunchMTparam_SuperMaster_CSK_Virunga_Asc_Full_Zoom1_ML23_KEEP_MassPro.txt
		LAUNCHPARAMDESC=LaunchMTparam_SuperMaster_CSK_Virunga_Desc_Full_Zoom1_ML23_MassPro.txt
	...
	###########################################
		MSBASDIR=${PATH_3602}/MSBAS/_${LABEL}_CSK_Auto_${BP}m_${BT}days
	...
	#################################################
		...
		DOUBLEDIFFPAIRSEWUD=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_VVP_S1_CSK.txt
		DOUBLEDIFFPAIRSASC=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_VVP_S1_CSK_Asc.txt
		DOUBLEDIFFPAIRSDESC=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_VVP_S1_CSK_Desc.txt
...
#		PATHLOCA=${PATH_3602}/MSBAS/zz_insets/${LABEL}
	...
	#########################################################################
	CRONJOB2=VVP_CSK_Step2_MassProc.sh
...
###########
	function PlotAll()
		...
		if [ "${EVENTS}" == "" ]
			then
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS_all_comp.sh _Auto_${ORDER}_${LAMBDA}_${LABEL} ${X1} ${Y1} ${X2} ${Y2} -f -r -t -g -start=20120310  # remove -f if does not want the linear fit
			else
				cp ${EVENTS}/All/Sat_Cover_VVP_S1_CSK.txt ${EVENTS}/Sat_Cover_${LABEL}.txt
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS_all_comp.sh _Auto_${ORDER}_${LAMBDA}_${LABEL} ${X1} ${Y1} ${X2} ${Y2} -f -r -g -t -events=${EVENTS} -start=20120310 # remove -f if does not want the linear fit			
		fi
...
#		rm -f ${MSBASDIR}/zz_UD_EW_TS_Auto_${ORDER}_${LAMBDA}_${LABEL}/${DESCRIPTION}_timeLines_${X1}_${Y1}_${X2}_${Y2}_Auto_${ORDER}_${LAMBDA}_${LABEL}.jpg
		rm -f ${EVENTS}/Sat_Cover_${LABEL}.txt
		}
	...
	function PlotAllNoCoh()
		...
		if [ "${EVENTS}" == "" ]
			then
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS_all_comp.sh _Auto_${ORDER}_${LAMBDA}_${LABEL}_NoCohThresh ${X1} ${Y1} ${X2} ${Y2} -f -r -t -g -start=20120310  # remove -f if does not want the linear fit etc..
			else
				cp ${EVENTS}/All/Sat_Cover_VVP_S1_CSK.txt ${EVENTS}/Sat_Cover_${LABEL}.txt
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS_all_comp.sh _Auto_${ORDER}_${LAMBDA}_${LABEL}_NoCohThresh ${X1} ${Y1} ${X2} ${Y2} -f -r -t -g -events=${EVENTS} -start=20120310 # remove -f if does not want the linear fit etc..		
		fi
...
#		rm -f ${MSBASDIR}/zz_UD_EW_TS_Auto_${ORDER}_${LAMBDA}_${LABEL}_NoCohThresh/${DESCRIPTION}_timeLines_${X1}_${Y1}_${X2}_${Y2}_Auto_${ORDER}_${LAMBDA}_${LABEL}_NoCohThresh.jpg
	...
	function PlotAllLOSasc()
		...
		if [ "${EVENTS}" == "" ]
			then
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS.sh ${X1} ${Y1} ${X2} ${Y2} -f -r -t -g -start=20120310 # remove -f if does not want the linear fit
			else
				cp ${EVENTS}/All/Sat_Cover_VVP_S1_CSK.txt ${EVENTS}/Sat_Cover_${LABEL}.txt
				${PATH_SCRIPTS}/SCRIPTS_MT/PlotTS.sh ${X1} ${Y1} ${X2} ${Y2} -f -r -t -g -events=${EVENTS} -start=20120310 # remove -f if does not want the linear fit etc..		
		fi
...
##################################################################
	...
	#LASTASC=`ls -lt ${CSKASC}/Geocoded/DefoInterpolx2Detrend | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'` # may be messing up if txt files are created for any other purpose in the dir... 
	#LASTDESC=`ls -lt ${CSKDESC}/Geocoded/DefoInterpolx2Detrend | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'`
	LASTASC=`find ${CSKASC}/Geocoded/DefoInterpolx2Detrend/ -maxdepth 1 -type f -name "*deg" -printf "%T+ %p\n" | sort -r | head -1 | ${PATHGNU}/gawk '{print $2}'`  
	LASTDESC=`find ${CSKDESC}/Geocoded/DefoInterpolx2Detrend/ -maxdepth 1 -type f -name "*deg" -printf "%T+ %p\n" | sort -r | head -1 | ${PATHGNU}/gawk '{print $2}'`
...
# Prepare MSBAS
###############
	${PATH_SCRIPTS}/SCRIPTS_MT/build_header_msbas_criteria.sh DefoInterpolx2Detrend 2 ${BP} ${BT} ${CSKASC} ${CSKDESC}
...
#         restrict_msbas_to_Coh.sh DefoInterpolx2Detrend1 ${COHRESTRICT} ${KMLCOH} ${CSKASC}/Geocoded/Coh
 #        restrict_msbas_to_Coh.sh DefoInterpolx2Detrend2 ${COHRESTRICT} ${KMLCOH} ${CSKDESC}/Geocoded/Coh
	...
	# Asc and Desc (with coh threshold restriction)
 	#-------------------------------------
		...
		# ASCENDING
			...
			${PATH_SCRIPTS}/SCRIPTS_MT/MSBAS.sh _Asc_Auto_${ORDER}_${LAMBDA}_${LABEL} ${TIMESERIESPTS} --msbasv4
