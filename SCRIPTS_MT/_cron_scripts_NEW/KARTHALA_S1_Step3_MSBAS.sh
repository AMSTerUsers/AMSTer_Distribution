...
# New in Distro V 2.6:  - Check time of last pair file : add /*deg at end of search line to be sure to check the images and not some txt file
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		LABEL=Karthala 	# Label for file naming (used for naming zz_ dirs with results and figs etc)
...
#		SET4=${PATH_1650}/SAR_SM/MSBAS/PF/set4
		...
		# WARNING: 	build_header_msbas_criteria.sh requires all table files with the same Bp and Bt names, hence one MUST link 
		#			SM table using 50m 50 days as tables named with 70m 70 days baselines
#		ln -s ${SET1}/table_0_50_0_150.txt ${SET1}/table_0_70_0_70.txt  2>/dev/null
#		ln -s ${SET2}/S/table_0_50_0_50.txt ${SET2}/table_0_70_0_70.txt 2>/dev/null
...
# 		DOUBLEDIFFPAIRSDESCSMIW=${PATH_SCRIPTS}/SCRIPTS_MT/_cron_scripts/List_DoubleDiff_EW_UD_${LABEL}.txt
	...
	#########################################################################
	CRONJOB2=KARTHALA_S1_Step2_MassProc_SM.sh
#	CRONJOB2SM=KARTHALA_S1_Step2_MassProc_SM.sh
...
##################################################################
	...
	#LASTASCSM=`ls -lt ${S1ASCSM}/Geocoded/${DEFOMODE}/*deg | head -n 2 | tail -n 1 | sed 's/.* //'` # may be messing up if txt files are created for any other purpose in the dir... 
