...
#mode SM
TABLEASC=$PATH_1650/SAR_SM/MSBAS/PF/set1/table_0_${BP}_0_${BT}_Till_${DATECHG}_0_${BP2}_0_${BT2}_After.txt
TABLEDESC=$PATH_1650/SAR_SM/MSBAS/PF/set2/table_0_${BP}_0_${BT}.txt

PARAMPROCESSASC=$PATH_1650/Param_files/S1/PF_SM_A_144/LaunchMTparam_S1_SM_Reunion_Asc_Zoom1_ML8_DEMGeoid.txt
PARAMPROCESSDESC=$PATH_1650/Param_files/S1/PF_SM_D_151/LaunchMTparam_S1_SM_Reunion_Desc_Zoom1_ML8_DEMGeoid.txt
...
CHECKREAD=`ps -eaf | ${PATHGNU}/grep PF_S1_Step1_Read_SMCoreg_Pairs_SM_DEMGeoid.sh | ${PATHGNU}/grep -v "grep " | grep -v "Crons_1_2_3.sh"  | wc -l`
	...
	then 
		...
		if [ ${CHECKASC} -lt 1 ] 
			then 
				...
				echo "Asc run on ${TODAY}"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_A_144/SMCrop_SM_20190808_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_Asc_last_MassRun.txt
				$PATH_SCRIPTS/SCRIPTS_MT/SuperMaster_MassProc.sh ${TABLEASC} ${PARAMPROCESSASC} > /dev/null 2>&1 &
			else 
				echo "Asc attempt aborted on ${TODAY} because other Mass Process in progress"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_A_144/SMCrop_SM_20190808_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_Asc_last_aborted.txt
		fi
		...
		if [ ${CHECKDESC} -lt 1 ] 
			then 
				# No process running yet
				echo "Desc run on ${TODAY}"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_D_151/SMCrop_SM_20181013_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_Desc_last_Mass.txt
				$PATH_SCRIPTS/SCRIPTS_MT/SuperMaster_MassProc.sh ${TABLEDESC} ${PARAMPROCESSDESC} > /dev/null 2>&1 &
			else 
				echo "Desc attempt aborted on ${TODAY} because other Mass Process in progress"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_D_151/SMCrop_SM_20181013_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_Desc_last_aborted.txt
		fi
	else 
		# VVP_S1_Step1_Read_SMCoreg_Pairs.sh is still running: abort and wait for tomorrow
		echo "Step2 aborted on ${TODAY} because PF_S1_Step1_Read_SMCoreg_Pairs_SM.sh is still running: wait for tomorrow"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_A_144/SMCrop_SM_20190808_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_aborted_because_Read_inProgress.txt
		echo "Step2 aborted on ${TODAY} because PF_S1_Step1_Read_SMCoreg_Pairs_SM.sh is still running: wait for tomorrow"  >>  $PATH_3610/SAR_MASSPROCESS/S1/PF_SM_D_151/SMCrop_SM_20181013_Reunion_-21.41--20.85_55.2-55.85_Zoom1_ML8/_aborted_because_Read_inProgress.txt
