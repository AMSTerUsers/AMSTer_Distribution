...
# New in Distro V 3.1 20230324 :	- remove Nyigo 2021 co-eruptive pairs (i.e. between 20210521 and 20210530) 
...
# New in Distro V 5.0.1 20240115: - missing closing bracket in testing file _NoCohThresh.eps
# New in Distro V 5.1.0 20240701: - enlarge orbital tube to 70m instead of 20 after 20220501
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		BP=70			# max perpendicular baseline 
		BT=400			# max temporal baseline

			# Not needed but good to remember: these are the Bp and Bt used for both data sets
			BP1=20
			BT1=400
			BP2=70
			BT2=400
	...
	###################################
		...
		S1ASC=${PATHMASSPROCESS}/S1/DRC_VVP_A_174/SMNoCrop_SM_${SMASC}_Zoom1_ML4
		S1DESC=${PATHMASSPROCESS}/S1/DRC_VVP_D_21/SMNoCrop_SM_${SMDESC}_Zoom1_ML4

		# Path to dir where list of compatible pairs files are computed (need one for each mode)
		SET1=${PATHSETI}/VVP/set6
		SET2=${PATHSETI}/VVP/set7

		# Path to LaunchParameters.txt files for each mode (need one for each mode) for Mass Processing
		LAUNCHPARAMASC=LaunchMTparam_S1_VVP_Asc_Zoom1_ML4_snaphu_square_MassPro.txt
		LAUNCHPARAMDESC=LaunchMTparam_S1_VVP_Desc21_Zoom1_ML4_snaphu_square_MassPro.txt
		...
		# They will be copied here below in DefoInterpolx2Detrendi
	...
	###########################################
		...
		mkdir -p ${MSBASDIR}/DefoInterpolx2Detrend1
		mkdir -p ${MSBASDIR}/DefoInterpolx2Detrend2
		
		cp ${PAIRSTOIGNORE1} ${MSBASDIR}/DefoInterpolx2Detrend1
		cp ${PAIRSTOIGNORE2} ${MSBASDIR}/DefoInterpolx2Detrend2
	...
	#########################################################################
	CRONJOB2=VVP_S1_Step2_MassProc.sh
...
##################################################################
	...
	#LASTASC=`ls -lt ${S1ASC}/Geocoded/DefoInterpolx2Detrend | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'` # may be messing up if txt files are created for any other purpose in the dir... 
	#LASTDESC=`ls -lt ${S1DESC}/Geocoded/DefoInterpolx2Detrend | ${PATHGNU}/grep -v ".txt" | head -n 2 | tail -n 1 | sed 's/.* //'`
	...
	# get date in sec of last available processed pairs in each MODE
	#LASTASCTIME=`stat -c %Y ${S1ASC}/Geocoded/DefoInterpolx2Detrend/${LASTASC}`
	#LASTDESCTIME=`stat -c %Y ${S1DESC}/Geocoded/DefoInterpolx2Detrend/${LASTDESC}`
...
# Remove lines in MSBAS/MODEi.txt file associated to possible broken links or duplicated lines with same name though wrong BP (e.g. after S1 orb update) 
		...
		#Need also for the _Full ones (that is without coh threshold)
		if [ ${IFCOH} == "YES" ] ; then 
			_Check_bad_DefoInterpolx2Detrend.sh DefoInterpolx2Detrend1_Full ${PATHMASSPROCESS} &
			_Check_bad_DefoInterpolx2Detrend.sh DefoInterpolx2Detrend2_Full ${PATHMASSPROCESS} &
			wait
...
###############
	...
	echo "Exclude co-eruptive pairs"
	# Get excluded pairs (cfr cron step 1)
	#cp ${PATHSETI}/VVP/set6/_EXCLUDE_PAIRS_ALTHOUGH_CRITERIA_OK.txt ${MSBASDIR}/DefoInterpolx2Detrend1/
	#cp ${PATHSETI}/VVP/set7/_EXCLUDE_PAIRS_ALTHOUGH_CRITERIA_OK.txt ${MSBASDIR}/DefoInterpolx2Detrend2/
	# Exclude them 
	Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/DefoInterpolx2Detrend1
	Exclude_Pairs_From_Mode.txt.sh ${MSBASDIR}/DefoInterpolx2Detrend2
	cd ${MSBASDIR}
