#!/bin/bash
# Script to run in cronjob for processing MUSTANG images with Coh Threshold:
...
#
	...
	########################		
		...
		if [ ${IFCOH} == "YES" ] 
			then 
				...
				KMLCOH=${PATH_1650}/kml/Nepal/gayutestcarre.kml
	...
	LABEL=Mustang_CohThresh${COHRESTRICT} 	# Label for file naming (used for naming zz_ dirs with results and figs etc)
	LABELDIR=Mustang 	# Label for Points time series and main MSBAS dir 
	eval LABELORIG=${LABELDIR}		# for later usage
	...
	###########################################
		MSBASDIR=${PATH_3610}/MSBAS/_${LABELDIR}_S1_Auto_Max${MAXSHORT}Shortests_CohThershold	
	...
	#################################################
		...
		TIMESERIESPTSDESCR=${PATH_1650}/Data_Points/Points_TS_${LABELDIR}.txt
		...
		DOUBLEDIFFPAIRSEWUD=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_${LABELDIR}.txt
 		DOUBLEDIFFPAIRSASCIW=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_${LABELDIR}.txt
 		DOUBLEDIFFPAIRSDESCIW=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_${LABELDIR}.txt
