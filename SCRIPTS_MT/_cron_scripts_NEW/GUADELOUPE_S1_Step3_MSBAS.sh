...
# New in Distro V 2.8:  - correct bug in searching for most recent DefoInterpolx2Detrend2.txt file 
# New in Distro V 2.9: - replace if -s as -f -s && -f to be compatible with mac os if 
...
# New in Distro V 4.1 20240415:	- increase Bp to 90m
# New in Distro V 4.2 20240729:	- take the correct tables of pairs
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	###################################
		...
		# IW
		S1ASCIW=$PATH_3601/SAR_MASSPROCESS/S1/GUADELOUPE_A_164/SMNoCrop_SM_20190622_Zoom1_ML2
		S1DESCIW=$PATH_3601/SAR_MASSPROCESS/S1/GUADELOUPE_D_54/SMNoCrop_SM_20200410_Zoom1_ML2 

		# Path to dir where list of compatible pairs files are computed (need one for each mode)
		SET1=${PATH_1650}/SAR_SM/MSBAS/GUADELOUPE/set1
		SET2=${PATH_1650}/SAR_SM/MSBAS/GUADELOUPE/set2


		# Path to LaunchParameters.txt files for each mode (need one for each mode)
		LAUNCHPARAMASCIW=LaunchMTparamS1_IW_Guadeloupe_A_Zoom1_ML2_MassProc.txt
		LAUNCHPARAMDESCIW=LaunchMTparamS1_IW_Guadeloupe_D_Zoom1_ML2_MassProc.txt
	...
	###########################################
		MSBASDIR=${PATH_3602}/MSBAS/_${LABEL}_S1_Auto_${BP}m_${BT}days	#_Till${DATECHG1}_${SET1BP2}m_After
	...
	#################################################
		...
		DOUBLEDIFFPAIRSEWUD=${PATH_1650}/Data_Points/List_DoubleDiff_EW_UD_${LABEL}.txt
 		DOUBLEDIFFPAIRSASCIW=${PATH_1650}/Data_Points/List_DoubleDiff_Asc_${LABEL}.txt
 		DOUBLEDIFFPAIRSDESCIW=${PATH_1650}/Data_Points/List_DoubleDiff_Desc_${LABEL}.txt
		
		
	# Name of previous cron jobs for the automatic processing of that target (used to check that no other process is runing)
	#########################################################################
	CRONJOB2=GUADELOUPE_S1_Step2_MassProc.sh
...
# 			fi 
 ...
 # Because in Guadeloupe the Desc mode was affected by a long interruption, one must restrict the msbas inversion to the common period: 
 
 echo " Because of data gap in Guadeloupe data before 20180601 in Descending mode, let's remove the data before that day in Descending mode"
 RemovePairsFromModeList_WithImagesBefore.sh DefoInterpolx2Detrend2.txt 20180601		
 # rename files accordingly 
 mv -f DefoInterpolx2Detrend2.txt DefoInterpolx2Detrend2_Full.txt
 FILECLEANED2=`ls -1tr DefoInterpolx2Detrend2.txt_After20180601*.txt | tail -1` # take the most recent if former run was not deleted
