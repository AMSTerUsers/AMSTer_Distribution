...
#
# New in Distro V 2.0 20251103:	- with ETAD
# New in Distro V 2.1 20251104:	- more detailed MSBAS dir naming 
#								- offer option to force inversion even if no new data
# New in Distro V 2.2.0 20251125 :	- always limited to 128 threads (see MAXTHREADS) to prevent problems with OPENBLAS, which is compiled by default for 128 threads 
#
#
...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		# 3D
		####
		FG=200 	# size of the filtering window in m (e.g. 10000, that is 10 km)
	...
	###################################
		...
		PATHMASSPROCESS=$PATH_3611/SAR_MASSPROCESS_ETAD
		...
		SET1=${PATHSETI}/MUSTANG/set11
		SET2=${PATHSETI}/MUSTANG/set12

		# Path to LaunchParameters.txt files for each mode (need one for each mode) for Mass Processing
		LAUNCHPARAMASC=LaunchMTparam_S1_Mustang_A_158_Zoom1_ML2_MassProc_0keep_ETAD.txt
		LAUNCHPARAMDESC=LaunchMTparam_S1_Mustang_D_19_Zoom1_ML2_MassProc_0keep_ETAD.txt
	...
	###########################################
		...
		MSBASDIR=${PATH_3610}/MSBAS/_${LABELDIR}_S1_Auto_${MSBASDIRCRITERIA}_ETAD
