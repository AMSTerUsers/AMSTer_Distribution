...
CHECKREAD=`ps -eaf | ${PATHGNU}/grep KARTHALA_S1_Step1_Read_SMCoreg_Pairs_SM.sh | ${PATHGNU}/grep -v "grep " | wc -l`
	...
	then 
		...
		if [ ${CHECKASC} -lt 1 ] 
			then 
				...
				echo "Asc run on ${TODAY}"  >>  $PATH_3601/SAR_MASSPROCESS/S1/KARTHALA_SM_A_86/SMCrop_SM_${SMASC}_ComoresIsland_-11.94--11.34_43.22-43.53_Zoom1_ML5/_Asc_last_MassRun.txt
				$PATH_SCRIPTS/SCRIPTS_MT/SuperMaster_MassProc.sh ${TABLEASC} ${PARAMPROCESSASC} > /dev/null 2>&1 &
			else 
				echo "Asc attempt aborted on ${TODAY} because other Mass Process in progress"  >>  $PATH_3601/SAR_MASSPROCESS/S1/KARTHALA_SM_A_86/SMCrop_SM_${SMASC}_ComoresIsland_-11.94--11.34_43.22-43.53_Zoom1_ML5/_Asc_last_aborted.txt
		fi
		# if riunning yet we will try egain tomorrow
 		...
 		if [ ${CHECKDESC} -lt 1 ] 
 			then 
 				...
 				echo "Desc run on ${TODAY}"  >>  $PATH_3601/SAR_MASSPROCESS/S1/KARTHALA_SM_D_35/SMCrop_SM_${SMDESC}_ComoresIsland_-11.94--11.34_43.22-43.53_Zoom1_ML5/_Desc_last_MassRun.txt
 				$PATH_SCRIPTS/SCRIPTS_MT/SuperMaster_MassProc.sh ${TABLEDESC} ${PARAMPROCESSDESC} > /dev/null 2>&1 &
 			else 
 				echo "Desc attempt aborted on ${TODAY} because other Mass Process in progress"  >>  $PATH_3601/SAR_MASSPROCESS/S1/KARTHALA_SM_D_35/SMCrop_SM_${SMDESC}_ComoresIsland_-11.94--11.34_43.22-43.53_Zoom1_ML5/_Desc_last_aborted.txt
 		fi
	else 
