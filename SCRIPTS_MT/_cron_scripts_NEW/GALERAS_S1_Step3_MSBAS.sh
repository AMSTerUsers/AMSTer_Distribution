...
# vvvvvvvvv Hard coded lines vvvvvvvvvvvvvv
	...
	#################
		...
		SET1=${PATH_1650}/SAR_SM/MSBAS/GALERAS/set1
		SET2=${PATH_1650}/SAR_SM/MSBAS/GALERAS/set2
		...
		LABEL=Galeras 				# Label for file naming (used for naming zz_ dirs with results and figs etc)
	...
	###################################
		...
		# IW
		S1ASCIW=$PATH_3601/SAR_MASSPROCESS/S1/GALERAS_A_120/SMNoCrop_SM_${SMASC}_Zoom1_ML2
		S1DESCIW=$PATH_3601/SAR_MASSPROCESS/S1/GALERAS_D_142/SMNoCrop_SM_${SMDESC}_Zoom1_ML2

		# Path to LaunchParameters.txt files for each mode (need one for each mode)
		LAUNCHPARAMASCIW=LaunchMTparam_S1_IW_Galeras_A_Zoom1_ML2_MassProc.txt
		LAUNCHPARAMDESCIW=LaunchMTparam_S1_IW_Galeras_D_Zoom1_ML2_MassProc.txt
	...
	###########################################
		MSBASDIR=${PATH_3601}/MSBAS/_${LABEL}_S1_Auto_${BP}m_${BT}days	#_Till${DATECHG1}_${SET1BP2}m_After
		# NOTE HERE UNUSUAL NAMING TO REMEMBER THAT IT WAS PREPRARED WITH DATA SETS WITH DIFFERENT MAX BT
		
	# Coherence restriction
	########################		
		IFCOH="YES"		# YES or NO  BEWARE: DO NOT RUN YES after NO or NO after YES without cleaning all the MSBAS dir before  
...
## Because in Galeras the Asc mode was affected by 2 long interruptions between ]20190502-20191110[ and ]20240505-20240809[, 
## one run here -if not done yet - a msbas inversion restricted to these 2 common periods: 

echo " Because of data gap in Galeras data between ]20190502-20191110[ and ]20240505-20240809[ in Ascending mode, let's split the processing"
