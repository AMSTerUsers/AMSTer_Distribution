#!/bin/bash
# Script to re-run the unwrapping of files from a given list of which must be in PAIR DIRS in SAR_MASSPROCESS. 
# Former unwrapped pahse is saved as unwrappedPhase.bak.
# If you have enough room on your MASSPROCESS disk, it might be wise to duplicate the dir where you will reprocess all the unwrappings ?
#
# You can then run a _ReGeoc_FromList.sh if required. 
#
...
#   RenamePath_Volumes.sh in .../RESAMPLED/...
# 
...
#
# Parameters:	- a list of dir where to re-run the unwrapping either in the form of S1A_174_20141017_A_S1A_174_20141110_A or 20141017_20141110
#               - File with the parameters needed for the run
#
# HARD CODED: 	- 
#
...
# New in Distro V 1.0.1:	- Cleaning
# New in Distro V 1.1.0:	- update to get same options as SinglePair.sh V_D4.0.0
...
# New in Distro V 4.3 20250224:	- typo in byte2float.py 
# New in Distro V 4.4 20250520:	- new param to define crop as GEO or SRA (lines and pixels) coordinates
...
DIRTOGEOC=$1	# a list of dir where to re-run the unwrapping either in the form of S1A_174_20141017_A_S1A_174_20141110_A or 20141017_20141110
...
=`GetParam "COORDSYST,"`			# COORDSYST, type of coordinates used to define crop: SRA (lines and pixels) or GEO
...
# Change DIRTOGEOC type if not in the form of S1A_174_20141017_A_S1A_174_20141110_A
if [ `head -1 ${DIRTOGEOC} | awk -F\_ '{print NF-1}'` == "1" ]    # count the nr of underscore. If only one, one must change format
	then 
...
do 
	...
	echo "*********************************************************"
	# rename former phase 
	RUNDIR=${MASSPROCESSPATH}/${RUNDIR}/
	UWPHASE=`find ${RUNDIR}/i12/InSARProducts/ -maxdepth 1 -type f -name "unwrappedPhase.??-??"`

	mv ${UWPHASE} /${RUNDIR}/i12/InSARProducts/unwrappedPhase.bak 2>/dev/null
	...
	if [ -f "${RUNDIR}/i12/TextFiles/snaphu.conf" ] && [ -s "${RUNDIR}/i12/TextFiles/snaphu.conf" ]
		then 
...
FILESTOGEOC=`echo "NO NO NO YES NO YES YES NO"`
	...
	case ${SATDIR} in
		...
		"CSK") 
			...
			;;
		"S1") 
			RESAMPLEDSLAVEDIR=`find ${RUNDIR}/i12/InSARProducts/ -maxdepth 1 -type d -name "*.csl"`
			if [ "${RESAMPLEDSLAVEDIR}" == "" ]
				then 
					UnwrapAndPlot 1 2   #  Parameters = ML factor for raster figures only (eg 5 1 for rectangle pixels)
				else 
					mkdir -p ${RESAMPLEDSLAVEDIR}/Data
					touch ${RESAMPLEDSLAVEDIR}/Data/Dummy.txt
					mkdir -p ${RESAMPLEDSLAVEDIR}/Headers
					touch ${RESAMPLEDSLAVEDIR}/Headers/Dummy.txt
					UnwrapAndPlot 1 2   #  Parameters = ML factor for raster figures only (eg 5 1 for rectangle pixels)
					rm -rf ${RESAMPLEDSLAVEDIR}/Data
					rm -rf ${RESAMPLEDSLAVEDIR}/Headers
			fi
			;;
		"ENVISAT") 
			UnwrapAndPlot 1 1   #  Parameters = ML factor for raster figures only (eg 5 1 for rectangle pixels)
