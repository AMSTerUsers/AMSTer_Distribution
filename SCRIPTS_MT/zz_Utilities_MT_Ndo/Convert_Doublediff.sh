...
# use gawk to convert the pixels coordinates
${PATHGNU}/gawk -v X0="$X0" -v Y0="$Y0" -v DX="$DX" -v DY="$DY" -v x0="$x0" -v y0="$y0" -v dx="$dx" -v dy="$dy" '{
    col1 = int((($1 * DX) + (X0 - x0)) / dx)
    col2 = int((($2 * DY) - (Y0 - y0)) / dy)
    col3 = int((($3 * DX) + (X0 - x0)) / dx)
    col4 = int((($4 * DY) - (Y0 - y0)) / dy)
    print col1, col2, col3, col4, $5
