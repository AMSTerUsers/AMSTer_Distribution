...
if [ "${MODE}" == "Ampli" ] 
	then 
		...
		#sort "${SOURCEDIR}"/List_Pairs_Searched_For_Ducplic.txt | uniq -c
		if [ ${NRDUPLIC} -gt 0 ]
			then 
				# count how many duplicated lines 
				#NRDUPLIC=`sort "${SOURCEDIR}"/List_Pairs_Searched_For_Ducplic.txt | uniq -cd | wc -l`
				...
				sort "${SOURCEDIR}"/List_Pairs_Searched_For_Ducplic.txt | uniq -cd > Duplicated_Files.txt
				echo "  //  Corresponding files are: "
		
				mkdir -p ___Duplicated_ToKill
		
				for DATES in `sort "${SOURCEDIR}"/List_Pairs_Searched_For_Ducplic.txt | uniq -d`
				...
				rm -f "${SOURCEDIR}"/List_Pairs_Searched_For_Ducplic.txt Duplicated_Files.txt 
			else 
				echo "  //   Directory ${SOURCEDIR} "
