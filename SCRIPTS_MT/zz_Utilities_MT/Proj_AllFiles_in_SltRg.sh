#!/bin/bash
# -----------------------------------------------------------------------------------------
# This script is aiming at projecting all the float files from a dir into the slant range  
#    of a given image. 
# BEWARE: files to be projected MUST be LatLong, WGS84 in degrees !!! 
#
# Parameters : - PRM date or S1 name (accept both date or name for S1)
#              - PRAMETERS file, incl path (e.g. /Users/doris/PROCESS/SCRIPTS_MT/___V20250808_LaunchMTparam.txt)
#			   - path to dir containing all float32 files in LatLong WGS84 to project in slantRange in ../SAR_SLC/SATDIR/TRKDIR/CropDir/img.slc/Data
#
# Dependencies:
#	- AMSTer Engine and AMSTer Engine Tools, at least V20260304
#	- PRAMETERS file
...
#	- linux trap function
#
# New in Distro V 1.0 20260326:		- Based on MasterDEM.txt
...
AUT="Nicolas d'Oreye, (c)2016-2019, Last modified on June 18, 2026"
...
DIRTOPROJ=$3				# Full path to dir containing all files to project in slant range

COMMENT=_TEMP_FOR_PROJ_SLTRG
...
if [ $# -lt 2 ] ; then echo " Usage $0 MAS PARAMETER_FILE "; exit 1; fi
...
IMGWITHDEM=${MASNAME} 	# = name of S1, or date if not S1 IW ;  same as SUPERMASNAME

# backup existing externalSlantRangeDEM.txt
cp -f ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM.txt ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM_${RUNDATE}_${RNDM1}.txt 2>/dev/null # mute error if file does not exist e.g. because of old version

	# do here what is normally done with fct "SlantRangeExtDEM PAIR BlankRunNo"    
	EchoTee "Slant range referencing of external DIR:"
	slantRangeDEM ${RUNDIR}/slantRange.txt -create  # slantRangeDEM projette n'importe quel fichier float geoprojeté en lat lon sur WGS84; Tous les fichiers trouvés dans un répertoire de référence seront projetés en slant range. 

	# update path to master.csl, DEM name and related info, reduction factor
	UpdateSlantRangeTXT "PAIR"    # because here MASDIR = SUPERMASDIR 

	EchoTee ""
	# ignore everything about mask and do not change existing masks in image dir 
	EchoTee ""


	# Run the slantRangeDEM here
	# Do not remove existing info about mask etc... 
	# Do not compute simulated amplitide with DEM

	# UPDATE INFO ABOUT DIR TO PROJ
	ChangeParam "External float files directory" "${DIRTOPROJ}/" slantRange.txt
...
echo "Last created AMSTer Engine source dir suggest projecting all files from DIR ${DIRTOPROJ} in slant range with ME version: ${LASTVERSIONMT}" > ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Projecting_DIRfiles_w_AMSTerEngine_V.txt
...
# set original externalSlantRangeDEM.txt file back 
cp -f ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM.txt ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM_SltRgDir.txt 
mv -f ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM_${RUNDATE}_${RNDM1}.txt ${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Info/externalSlantRangeDEM.txt 
...
case ${OS} in 
	"Linux") 
		if [ -f "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] && [ -s "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] 
			then 
				espeak " Files in DIR projected in slant range. Hope it worked." 
			else 
				espeak " Files in DIR not projected in slant range  ? Check..." 
	...
	"Darwin")
		if [ -f "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] && [ -s "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] 
			then 
				say " Files in DIR projected in slant range. Hope it worked." 	
			else 
				say " iles in DIR not projected in slant range  ? Check..." 	
		fi
		;;
	*)
		if [ -f "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] && [ -s "${DATAPATH}/${SATDIR}/${TRKDIR}/${CROPDIR}/${MASDIR}/Data/externalSlantRangeDEM" ] 
			then 
				echo " Files in DIR projected in slant range. Hope it worked." 	
			else 
				echo " iles in DIR not projected in slant range  ? Check..." 	
		fi
		;;
esac			
