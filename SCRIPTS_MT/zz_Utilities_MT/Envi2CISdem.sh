...
# New in Distro V 2.1 20250402:	- make grep search case insensitive
#
...
AUT="Nicolas d'Oreye, (c)2016-2019, Last modified on Apr 02, 2025"
echo " "
echo "${PRG} ${VER}, ${AUT}"
echo " "
...
if [ `cat ${ENVIDEM}.hdr | grep -i Lines | wc -l ` -eq 1 ] 
	then 
		NLINES=`cat ${ENVIDEM}.hdr | grep -i Lines | cut -d = -f 2`   
	else 
		NLINES=`cat ${ENVIDEM}.hdr | grep -i lines | cut -d = -f 2`   
fi
