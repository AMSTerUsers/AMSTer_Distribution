...
#
# REMEMBER that MasTerEngine takes in priority the txt header file if both txt and hdr are present. 
#
...
AUT="N. d'Oreye, (c)2022, Last modified on Aug 30, 2023"
echo " "
echo "${PRG} ${VER}, ${AUT}"
echo " "
...
#### However, MasTerEngine consider the middle of the pixel as the reference instead of the corner of the pixel, hence we must remove half of pix resolution
...
while true; do
	...
	case $EGQ in
		"E")
			echo ""
			echo "OK, you confirm that your DEM is already referred to ellipsoidal height, which is what MasTerEngin expects. I set it in the .txt file..."
		...
		"Q")
			...
			echo "I add nothing about that in the .txt file, which means that MasTerEngine will consider "
			echo "  that it is NOT corrected from geoidal height yet. In any case MasTer Engine will work but you might have"
